...
│   │   ├── render.py        # 飞书卡片markdown生成 (新游+排名+市场+热点, 含_split_markdown_entries共享拆分器)
...
│   │   └── deep_researcher.py # 深度研究Agent: 问题拆解+并行搜索+交叉验证+500字引用简报
│   ├── pipeline/         # 纯规则管道
...
│   │   ├── web_search.py       # 搜索引擎: 360/Sogou 新闻+网页搜索(含缓存)
│   │   ├── cross_validate.py   # 交叉验证: 对claim搜其他来源验证corroborating/contradicting
│   │   ├── db_query.py         # DB查询工具
│   │   └── image_fetch.py      # 图片获取
...
```
Phase 0A: Scrape  — 并行抓取 6个scraper 
...
Phase 0.5: Hot Keywords — 5 源并行 (百度热搜 + 知乎热榜 + 微博热搜 + 小红书聚合 + 预设兴趣词)
Phase 1:  Differ → StoryPicker (纯规则, 0 token)
Phase 1.5: Hot Tracker — Agent 驱动热点筛选+摘要 (tool loop + web_fetch)
...
- **热点追踪**: Hot Tracker Agent — （ tool loop + web_fetch 工具）。从搜索结果中语义筛选 ≤7 条最有价值的行业热点，为每条生成 AI 摘要。渲染端 `build_hot_topic_elements()` 逐条渲染：独立 markdown block + `感兴趣` 按钮紧跟。Agent 失败时回退到搜索顺序前 7 条。上限 7 条。
...
- **评测基准** — 人工标注 30 天日报黄金数据集，建 Precision@7 / Recall / nDCG@7 自动化评测，接入 CI 做每次 commit 的质量回归检测。详见 `docs/DEEP_RESEARCH_AGENT.md` 末尾。
- **Deep Research Agent** ✅ — 多步自主研究 Agent，含问题拆解 + 并行搜索 + 对抗验证（`cross_validate`）+ 引用溯源。已实现：`src/agents/deep_researcher.py`，触发方式：`--deep-research` CLI + bot 自动（≥3 次"感兴趣"）。详见 [docs/DEEP_RESEARCH_AGENT.md](docs/DEEP_RESEARCH_AGENT.md)。
...
- After any code change, always perform a self-review before sending the final response.
- Review focus must include correctness, edge cases, regression risk, consistency with existing project patterns, and missing tests.
- If findings exist, list findings first ordered by severity, with file references and brief user impact.
- If no findings exist, explicitly say no findings were found and mention residual risks or testing gaps.
