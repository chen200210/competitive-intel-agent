...
> **注意**：本文档为设计原文。实际实现见 `src/storage/sqlite.py`（DDL + CRUD）和 `src/agents/base.py`（`_execute_tool` 审计日志写入）。搜索引擎已改为 Tavily 优先、Bing/DDG fallback，详见 `src/tools/web_search.py`。
...
| `query` | 原始搜索词 |
| `engine` | `tavily` / `bing` / `ddg` |
| `results_json` | 返回的 `[{title, url, snippet, rank}]` |
| `result_count` | 命中了多少条 |
| `called_by` | 哪个 Agent 调的（`overview_scanner` / `researcher` / ...） |
| `searched_at` | 搜索时间 |

**逻辑**：Agent 调 `web_search` → 先查 `search_cache` → 命中且距今 <24h → 直接返回缓存结果 → 没命中 → Tavily 搜（fallback: Bing → DDG）→ 写入缓存。
